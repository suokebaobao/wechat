...
class Uploads extends UserBase
{
    ...
    public function uploadImage()
    {
        try {
            ...
            //判断上传位置
            if($params['type']=='1'){//七牛
                $key = 'shop_'.ShopId().'/'.date('Y/md/His_').substr(microtime(), 2, 6).'_'.mt_rand(0,999).'.'.$file->getOriginalExtension();
    ...
    */
    ...
    private function saveRemote($config,$fieldName)
    {
        ...
        $dirname = './uploads/shop_'. ShopId().'/remote/'.date('Ymd').'/';
        $file['oriName'] = $m ? $m[1] : "";
        ...
        if(!(file_put_contents($fullName, $img) && file_exists($fullName))){ //移动失败
            ...
            UploadsModel::CreateInfo('local','', $file['oriName'], $file['filesize'], $file['oriName']);
            $data=array(
                'state' => 'SUCCESS',
    ...
    */
    private function upBase64($config,$fieldName){
        ...
        $data=array(
            ...
            UploadsModel::CreateInfo('0', $file['oriName'], $file['filesize'], $file['oriName'],substr($file['fullName'],1));
            $data=array(
                'state' => 'SUCCESS',
